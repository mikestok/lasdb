...
(function() {
  jQuery(document).ready(function($) {
    var answer_changers, answer_movers, radios;
    radios = "table.answers tr.answer input[@type='checkbox'].radio-like";
    $(document).delegate(radios, "click", function(event) {
      event.stopPropagation();
      $(radios).attr("checked", null);
      return $(event.target).attr("checked", 1);
    });
    answer_changers = "table.answers .change-answer-content";
    answer_movers = "table.answers .move-answer-control";
    $(document).delegate(answer_changers, "change", function(event) {
      return $(answer_movers).hide();
    });
    $.setUpAutoResize = function() {
      return $("form.edit_question textarea").autoResize({
        extraSpace: 5
      });
    };
    return $.setUpAutoResize();
  });
